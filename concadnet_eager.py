...
with tf.device("/cpu:0"):
    ...
    test_dataset = tf.data.TFRecordDataset(val_dataset_file)
...
class ConCaDNet(tfe.Network):
    ...
    def __init__(self):
        ...
        self.l1_1 = self.track_layer(tf.layers.Conv2D(2, 3, padding="SAME", name="Conv_1_1", activation=tf.nn.leaky_relu))
        self.l1_2 = self.track_layer(tf.layers.Conv2D(4, 3, padding="SAME", name="Conv_1_2", activation=tf.nn.leaky_relu))
        self.l1_3 = self.track_layer(tf.layers.Conv2D(6, 3, padding="SAME", name="Conv_1_3", activation=tf.nn.leaky_relu))
        ...
        self.l2_1 = self.track_layer(tf.layers.Conv2D(2, 5, padding="SAME", name="Conv_2_1", activation=tf.nn.leaky_relu))
        self.l2_2 = self.track_layer(tf.layers.Conv2D(4, 5, padding="SAME", name="Conv_2_2", activation=tf.nn.leaky_relu))
        self.l2_3 = self.track_layer(tf.layers.Conv2D(6, 5, padding="SAME", name="Conv_2_3", activation=tf.nn.leaky_relu))
        ...
        self.l3_1 = self.track_layer(tf.layers.Conv2D(4, 3, padding="SAME", name="Conv_3_1", activation=tf.nn.leaky_relu))
        self.l3_2 = self.track_layer(tf.layers.Conv2D(6, 3, padding="SAME", name="Conv_3_2", activation=tf.nn.leaky_relu))
        self.l3_3 = self.track_layer(tf.layers.Conv2D(8, 3, padding="SAME", name="Conv_3_3", activation=tf.nn.leaky_relu))
        self.l3_4 = self.track_layer(tf.layers.Conv2D(10, 3, padding="SAME", name="Conv_3_4", activation=tf.nn.leaky_relu))
        self.l3_mp = self.track_layer(tf.layers.MaxPooling2D(3, 2, padding="SAME", name="MaxPool_3"))
        self.l4_1 = self.track_layer(tf.layers.Conv2D(4, 5, padding="SAME", name="Conv_4_1", activation=tf.nn.leaky_relu))
        self.l4_2 = self.track_layer(tf.layers.Conv2D(6, 5, padding="SAME", name="Conv_4_2", activation=tf.nn.leaky_relu))
        self.l4_3 = self.track_layer(tf.layers.Conv2D(8, 5, padding="SAME", name="Conv_4_3", activation=tf.nn.leaky_relu))
        self.l4_4 = self.track_layer(tf.layers.Conv2D(10, 5, padding="SAME", name="Conv_4_4", activation=tf.nn.leaky_relu))
    ...
    def call(self, inputs, training=True):
        ...
        if not training:
            self.display_layers(inputs, [l1_1, l1_2, l1_3, l1_4, l2_1, l2_2, l2_3, l2_4, l3_1, l3_2, l3_3, l3_4,
                                         l4_1, l4_2, l4_3, l4_4])
        return conv
    ...
    with tf.device("/GPU:0"):
        for (batch, (x, y, s, d)) in enumerate(tfe.Iterator(ds)):
            ...
            if batch%log_interval == 0:
                ...
                _ = saver.save("../Models/"+str(model_version)+str(run_number)+str(batch))
...
def evaluate(model):
    ...
    with tf.device("/GPU:0"):
        ...
        print("Validation Loss: {}, Validation AUC: {}, Testing Loss: {}, Testing AUC: {}".format(vl, va, tl, ta))
